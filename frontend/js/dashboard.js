...
/* Ask Campus Board close/reopen behavior */
document.addEventListener("DOMContentLoaded", () => {
  const askBoard = document.querySelector(
    "#askBoard, #ask-board, .ask-board, .ask-campus-board, [class*='ask-board']"
  );
  const askClose = document.getElementById("ask-board-close");
  const askTrigger = document.querySelector(
    "#askBoardBtn, #ask-board-btn, #ask-board-trigger, .ask-board-trigger, [class*='ask-board-trigger']"
  );

  if (askClose) {
    askClose.addEventListener("click", (event) => {
      event.preventDefault();
      event.stopPropagation();

      const target = askClose.closest(
        "#askBoard, #ask-board, .ask-board, .ask-campus-board, [class*='ask-board']"
      ) || askBoard;

      if (target) {
        target.classList.add("ask-board-hidden");
  ...
  if (askTrigger) {
    askTrigger.addEventListener("click", () => {
      const target = askBoard;
      if (target && target.classList.contains("ask-board-hidden")) {
        target.classList.remove("ask-board-hidden");
