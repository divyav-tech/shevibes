...
(function (global) {
  ...
  var STORAGE_KEYS = {
    ...
    deletedNotices: "campusboard.deletedNoticeIds",
    deletedOpportunities: "campusboard.deletedOpportunityIds",
    deletedEvents: "campusboard.deletedEventIds",
    deletedTimetables: "campusboard.deletedTimetableIds",
  ...
  function getDeletedNoticeIds() {
    try {
      var raw = localStorage.getItem(STORAGE_KEYS.deletedNotices);
      return raw ? JSON.parse(raw) : [];
    } catch (e) {
      return [];
    }
  ...
  function markNoticeDeleted(id) {
    if (id == null) return;
    var list = getDeletedNoticeIds();
    var strId = String(id);
    if (list.indexOf(strId) === -1) {
      list.push(strId);
      localStorage.setItem(STORAGE_KEYS.deletedNotices, JSON.stringify(list));
    }
    return list;
  }

  function isNoticeDeleted(id) {
    if (id == null) return false;
    return getDeletedNoticeIds().indexOf(String(id)) !== -1;
  }
  ...
  function getCrAnnouncements() {
    try {
      ...
      var deleted = getDeletedNoticeIds();
      return list.filter(function(a) {
        return deleted.indexOf(String(a.id)) === -1;
      });
    } catch (e) {
      return [];
    }
  ...
  function deleteCrAnnouncement(id) {
    if (id == null) return getCrAnnouncements();
    var strId = String(id);
    markNoticeDeleted(strId);
    try {
      var raw = localStorage.getItem(STORAGE_KEYS.crAnnouncements);
      var list = raw ? JSON.parse(raw) : [];
      var filtered = list.filter(function (a) {
        return String(a.id) !== strId;
      });
      localStorage.setItem(STORAGE_KEYS.crAnnouncements, JSON.stringify(filtered));
      return filtered;
    } catch (e) {
      return [];
    }
  ...
  function isIdDeleted(key, id) {
    if (id == null) return false;
    return getDeletedIds(key).indexOf(String(id)) !== -1;
  }

  function markOpportunityDeleted(id) { return markIdDeleted(STORAGE_KEYS.deletedOpportunities, id); }
  function isOpportunityDeleted(id) { return isIdDeleted(STORAGE_KEYS.deletedOpportunities, id); }
  function markEventDeleted(id) { return markIdDeleted(STORAGE_KEYS.deletedEvents, id); }
  function isEventDeleted(id) { return isIdDeleted(STORAGE_KEYS.deletedEvents, id); }
  function markTimetableDeleted(id) { return markIdDeleted(STORAGE_KEYS.deletedTimetables, id); }
  function isTimetableDeleted(id) { return isIdDeleted(STORAGE_KEYS.deletedTimetables, id); }
  ...
  function getAllAnnouncements() {
    ...
    var deleted = getDeletedNoticeIds();
    return crPosted.concat(SAMPLE_ANNOUNCEMENTS).filter(function (a) {
      return deleted.indexOf(String(a.id)) === -1;
    });
  }
  ...
  function getAllOpportunities() {
    var deleted = getDeletedIds(STORAGE_KEYS.deletedOpportunities);
    return SAMPLE_OPPORTUNITIES.filter(function (o) {
      return deleted.indexOf(String(o.id)) === -1;
    });
  }

  function getAllEvents() {
    var deleted = getDeletedIds(STORAGE_KEYS.deletedEvents);
    return SAMPLE_EVENTS.concat(deriveEventsFromCrAnnouncements()).filter(function (e) {
      return deleted.indexOf(String(e.id)) === -1;
    });
  }
  ...
  function isCr(profile) {
    if (!profile) return false;
    var r = String(profile.role || "").toLowerCase();
    return r === "cr" || r === "class representative" || r.indexOf("representative") !== -1;
  }

  function canDeleteAnnouncement(item, profile) {
    if (!profile || !isCr(profile)) return false;
    if (!item) return false;
    if (item.posted_by && String(item.posted_by) === String(profile.id)) return true;
    if (item.id && String(item.id).indexOf("cr-") === 0) return true;
    if (item.forClass && audienceMatches(item.forClass, profile)) return true;
    if (item.source && item.source.indexOf("CR") !== -1 && (!item.forClass || audienceMatches(item.forClass, profile))) return true;
    return false;
  }

  function canDeleteOpportunity(item, profile) {
    if (!profile || !isCr(profile) || !item) return false;
    if (item.posted_by && String(item.posted_by) === String(profile.id)) return true;
    if (item.posted_by == null || item.posted_by === "") return true;
    return false;
  }

  function canDeleteEvent(item, profile) {
    if (!profile || !isCr(profile) || !item) return false;
    var cat = String(item.category || item.type || "").toLowerCase();
    if (cat === "personal" || item.source === "My note") return false;
    if (item.created_by && String(item.created_by) === String(profile.id)) return true;
    if (item.created_by == null || item.created_by === "") return true;
    if (item.aiGenerated && audienceMatches(item.forClass, profile)) return true;
    return false;
  }

  function canDeleteContent(item, profile, type) {
    if (type === "announcement") return canDeleteAnnouncement(item, profile);
    if (type === "opportunity") return canDeleteOpportunity(item, profile);
    if (type === "event") return canDeleteEvent(item, profile);
    return false;
  }

  function closeAllOverflowMenus() {
    document.querySelectorAll(".card-overflow.is-open").forEach(function (wrap) {
      wrap.classList.remove("is-open");
      var btn = wrap.querySelector("[data-overflow-toggle]");
      var menu = wrap.querySelector(".card-overflow-menu");
      if (btn) btn.setAttribute("aria-expanded", "false");
      if (menu) menu.hidden = true;
    });
  }

  function overflowMenuHtml(type, itemId) {
    return (
      '<div class="card-overflow">' +
        '<button class="card-overflow-btn" type="button" data-overflow-toggle="' + type + ':' + itemId + '" aria-haspopup="true" aria-expanded="false" aria-label="More actions">⋮</button>' +
        '<div class="card-overflow-menu" role="menu" hidden>' +
          '<button class="card-overflow-item" type="button" role="menuitem" data-overflow-delete="' + type + ':' + itemId + '">Delete</button>' +
        '</div>' +
      '</div>'
    );
  }

  function bindOverflowMenuOnce() {
    if (bindOverflowMenuOnce.bound) return;
    bindOverflowMenuOnce.bound = true;
    document.addEventListener("click", function (e) {
      var toggle = e.target.closest("[data-overflow-toggle]");
      if (toggle) {
        e.preventDefault();
        e.stopPropagation();
        var wrap = toggle.closest(".card-overflow");
      ...
      if (!e.target.closest(".card-overflow-menu")) {
        closeAllOverflowMenus();
      }
    });
  ...
  function ensureDeleteConfirmModal() {
    var modal = document.getElementById("cb-delete-confirm-modal");
    if (modal) return modal;
    modal = document.createElement("div");
    modal.className = "cb-modal";
    modal.id = "cb-delete-confirm-modal";
    modal.setAttribute("aria-hidden", "true");
    modal.innerHTML =
      '<div class="cb-modal-backdrop" data-close-delete-modal></div>' +
      '<div class="cb-modal-panel cb-delete-confirm-panel" role="dialog" aria-modal="true" aria-labelledby="delete-modal-heading">' +
        '<button class="cb-modal-close" type="button" aria-label="Close" data-close-delete-modal>×</button>' +
        '<div class="cb-delete-icon-wrap" aria-hidden="true">🗑️</div>' +
        '<h3 class="cb-delete-title" id="delete-modal-heading">Delete Announcement</h3>' +
        '<p class="cb-delete-message">Are you sure you want to delete this announcement?</p>' +
        '<div class="cb-delete-item-title" id="delete-modal-item-title"></div>' +
        '<div class="cb-delete-modal-actions">' +
          '<button class="btn btn-ghost" type="button" data-close-delete-modal id="delete-cancel-btn">Cancel</button>' +
          '<button class="btn btn-danger" type="button" id="delete-confirm-btn">Delete</button>' +
        '</div>' +
      '</div>';
    document.body.appendChild(modal);

    modal.querySelectorAll("[data-close-delete-modal]").forEach(function (btn) {
  ...
  function confirmDeleteContent(item, options) {
    options = options || {};
    var kind = options.kind || "announcement";
    var copy = {
      announcement: {
        title: "Delete Announcement",
        message: "Are you sure you want to delete this announcement?",
        success: "Announcement deleted successfully.",
        fail: "Could not delete announcement. Please try again.",
        eventName: "cb:announcement-deleted",
        savedType: "announcement"
      },
      opportunity: {
        title: "Delete Opportunity",
        message: "Are you sure you want to delete this opportunity?",
        success: "Opportunity deleted successfully.",
        fail: "Could not delete opportunity. Please try again.",
        eventName: "cb:opportunity-deleted",
      ...
      event: {
        ...
        fail: "Could not delete event. Please try again.",
        eventName: "cb:event-deleted",
        savedType: "event"
      },
      timetable: {
        ...
        fail: "Could not delete timetable. Please try again.",
        eventName: "cb:timetable-deleted",
        savedType: null
      }
    }[kind] || {
      title: "Delete",
      ...
      eventName: "cb:content-deleted",
      savedType: kind
    };

    var deleteRequest = options.deleteRequest;
    if (!deleteRequest) {
      if (kind === "opportunity") deleteRequest = function () { return api.deleteOpportunity(item.id); };
      else if (kind === "event") deleteRequest = function () { return api.deleteCalendarEvent(item.id); };
      else if (kind === "timetable") deleteRequest = function () { return api.deleteTimetable(item.id); };
      else deleteRequest = function () { return api.deleteNotice(item.id); };
    }

    var modal = ensureDeleteConfirmModal();
    var headingEl = document.getElementById("delete-modal-heading");
    var messageEl = modal.querySelector(".cb-delete-message");
    var titleEl = document.getElementById("delete-modal-item-title");
    ...
    if (titleEl) titleEl.textContent = item.title || ("Untitled " + (kind.charAt(0).toUpperCase() + kind.slice(1)));
    ...
    newConfirmBtn.addEventListener("click", function () {
      ...
      deleteRequest().then(function (res) {
        if (!res || !res.success) {
          newConfirmBtn.disabled = false;
          newConfirmBtn.textContent = "Delete";
          toast((res && res.error) || copy.fail);
          return;
        }
        ...
        if (kind === "announcement") {
          deleteCrAnnouncement(item.id);
          markNoticeDeleted(item.id);
        } else if (kind === "opportunity") {
          markOpportunityDeleted(item.id);
        } else if (kind === "event") {
          markEventDeleted(item.id);
        } else if (kind === "timetable") {
          markTimetableDeleted(item.id);
        }

        if (copy.savedType && isSaved(copy.savedType, item.id)) {
          toggleSaved(copy.savedType, item.id);
          document.dispatchEvent(new CustomEvent("cb:saved-changed"));
        }

        document.dispatchEvent(new CustomEvent(copy.eventName, { detail: { id: String(item.id), kind: kind } }));

        var detailModal = document.getElementById("cb-shared-detail-modal");
  ...
  function confirmDeleteAnnouncement(item, onConfirmed) {
    confirmDeleteContent(item, { kind: "announcement", onConfirmed: onConfirmed });
  }
  ...
  function openDetailModal(item) {
    ...
    var profile = getProfile();
    var canDel = canDeleteContent(item, profile, savedType);
    var overflowBtn = canDel ? overflowMenuHtml(savedType, item.id) : "";
    ...
    body.innerHTML =
      ...
      '<div class="detail-actions" style="margin-top:22px;display:flex;gap:10px;flex-wrap:wrap;">' +
    ...
    bindOverflowMenuOnce();
    if (canDel) {
      var detailDelBtn = body.querySelector("[data-overflow-delete]");
      if (detailDelBtn) {
        detailDelBtn.addEventListener("click", function (e) {
          e.preventDefault();
          e.stopPropagation();
          closeAllOverflowMenus();
          confirmDeleteContent(item, { kind: savedType });
        });
      }
    }
  ...
  function renderInfoCards(containerId, items, type, toneClass, emptyMessage) {
    ...
    items.forEach(function (item) {
      ...
      var completeButton = taskIsActionable(item)
        ...
        : '';
      var canDel = canDeleteContent(item, profile, type);
      var overflow = canDel ? overflowMenuHtml(type, item.id) : "";
      card.innerHTML =
      ...
      card.addEventListener("click", function (e) {
        ...
        if (e.target.closest("[data-overflow-toggle]")) return;
        if (e.target.closest("[data-overflow-delete]")) return;
        item.savedType = type;
    ...
    bindOverflowMenuOnce();
    container.querySelectorAll("[data-overflow-delete]").forEach(function (btn) {
      btn.addEventListener("click", function (e) {
        e.preventDefault();
        e.stopPropagation();
        closeAllOverflowMenus();
        var parts = (btn.dataset.overflowDelete || "").split(":");
        var kind = parts[0];
        var targetId = parts.slice(1).join(":");
        var targetItem = items.find(function(it) { return String(it.id) === String(targetId); }) || { id: targetId, title: kind };
        confirmDeleteContent(targetItem, {
          kind: kind,
  ...
  var api = {
          ...
          return r.json().then(function(data) {
            if (!r.ok) {
              return {
                success: false,
                status: r.status,
                error: data.message || data.error || "Failed to delete announcement"
              };
            }
            return Object.assign({ success: true, status: r.status }, data);
          }).catch(function() {
            if (!r.ok) {
              return { success: false, status: r.status, error: "Failed to delete announcement (status " + r.status + ")" };
            }
            return { success: true };
          });
        })
    ...
    deleteOpportunity: function(id) {
      return fetch(API_BASE + "/api/opportunities/" + encodeURIComponent(id), {
        method: "DELETE",
        credentials: "include"
      })
        .then(function(r) {
          return r.json().then(function(data) {
            if (!r.ok) {
              return {
                success: false,
                status: r.status,
                error: data.message || data.error || "Failed to delete opportunity"
              };
            }
            return Object.assign({ success: true, status: r.status }, data);
          }).catch(function() {
            if (!r.ok) {
              return { success: false, status: r.status, error: "Failed to delete opportunity (status " + r.status + ")" };
            }
            return { success: true };
          });
        })
  ...
  global.CB = {
    ...
    storage: {
      ...
      deleteCrAnnouncement: deleteCrAnnouncement,
      markNoticeDeleted: markNoticeDeleted,
      isNoticeDeleted: isNoticeDeleted,
      getDeletedNoticeIds: getDeletedNoticeIds,
